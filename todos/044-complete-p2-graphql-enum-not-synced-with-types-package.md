...
This duplicates the values from `SUPPORTED_LOCALES` in `@motovault/types`. If a new locale is added to `SUPPORTED_LOCALES`, the developer must remember to also update this enum. There is no compile-time or runtime check that they stay in sync.

-- Migration: Add currency preference to users and per-expense currency tracking
-- Supports 15 popular currencies with USD as default

-- 1. Add currency column to users table (display preference)
...
COMMENT ON COLUMN public.users.currency IS 'User display preference for currency symbol/formatting (ISO 4217 code)';

-- 2. Add currency column to expenses table (records currency at time of entry)
...
COMMENT ON COLUMN public.expenses.currency IS 'Currency the expense was entered in (ISO 4217 code, set from user preference at time of entry)';

-- 3. Drop old complete_onboarding overload (17 params) before creating new 18-param version
...
-- 4. Recreate complete_onboarding with p_currency parameter

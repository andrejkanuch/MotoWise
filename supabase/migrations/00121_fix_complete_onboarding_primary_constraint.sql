-- Fix: complete_onboarding fails with duplicate primary constraint
-- when user already has a primary motorcycle (e.g. re-onboarding after reset).
-- Solution: unset existing primary before inserting the new bike.

-- Drop the current 18-param signature
DROP FUNCTION IF EXISTS public.complete_onboarding(
  UUID, JSONB, TEXT, TEXT, INTEGER, motorcycle_type, INTEGER, TEXT, TEXT,
  TEXT, BOOLEAN, TEXT, BOOLEAN, BOOLEAN, BOOLEAN, TEXT, TEXT, TEXT
);
...
BEGIN
  ...
  -- Self-healing: ensure public.users row exists
  ...
  -- Build extra preferences from parameters
  ...
  -- Merge preferences into user record
  UPDATE public.users
  ...
  IF p_bike_make IS NOT NULL AND p_bike_model IS NOT NULL AND p_bike_year IS NOT NULL THEN
    ...
    IF v_bike_id IS NULL THEN
      -- Unset any existing primary bike before inserting new one

...
- Runtime version policy is `appVersion` (currently `3.18.0`), so OTA updates only reach builds with matching app version.
...
- **Web / Vercel build config**: `apps/web/vercel.json` is authoritative. Its `ignoreCommand` (turbo-ignore) **overrides** the dashboard's "Ignored Build Step" — a custom command set there is silently dead config, which is why a project-level `[ "$VERCEL_ENV" != "production" ]` never stopped preview builds (removed 2026-08-05). Previews are wanted: verifying the 404 contract needs a real deployment (`pnpm check:404`). Change the rule in `vercel.json`, in review — not in the dashboard. `vercel.json` rejects unknown keys, so it cannot carry comments.
- **Web / 404 contract**: never add a `loading.tsx` above a route that calls `notFound()`, `redirect()` or `permanentRedirect()` — the Suspense boundary streams the shell first, and a streamed response can only carry HTTP 200, silently turning every 404 into an indexable soft-404. Guarded by `apps/web/src/app/__tests__/not-found-contract.test.ts` (CI) and `pnpm check:404` (needs a real server; dev mode cannot reproduce this). See `docs/solutions/runtime-errors/nextjs-streaming-swallows-404s-and-redirects.md`.

...
export function RouteDetailReviews({
  reviews,
  totalCount,
  hasMore,
  isAuthenticated,
}: RouteDetailReviewsProps) {
  ...
  const handleSignUp = () => {
    router.push('/signup?source=review_softwall');
  };

  return (
    <section className="mt-12">
      <h2 className="text-xl font-bold mb-6" style={{ color: palette.white }}>
        Reviews ({totalCount})
      </h2>
      ...
      {/* Soft wall for anonymous users when there are more reviews */}
      {showSoftWall && (
        <div className="relative">
          {/* Blurred placeholder content to hint at hidden reviews (SEO-safe:
              the actual review text is in JSON-LD, not duplicated here) */}
          <div className="select-none" aria-hidden="true" style={{ filter: 'blur(8px)' }}>
            {/* Render 2 skeleton review cards as blurred teasers */}
            {[1, 2].map((i) => (
              <div
                key={i}
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex gap-0.5">
                    {Array.from({ length: 5 }).map((_, j) => (
                      <div
                        // biome-ignore lint/suspicious/noArrayIndexKey: static star skeleton
                        key={j}

 ...
 *
 * Reads only data the app already has (trip, waypoints, bike, fuel) and
 * produces a deterministic report. Not a web-service call — cheap to
 * recompute whenever the underlying data changes so the ring can
 * animate in real-time as the rider adds stops / fills in details.
 *
 * Keep this file free of React Native imports so it's testable in Node.
 */
...
export interface ReadinessInputTrip {
  ...
  /** Straight-line distance in metres between consecutive stops when route
   *  geometry isn't available. Passed in pre-computed so we don't have to
   *  reimplement haversine here. Optional. */
...
function computeLegDistancesKm(
  waypoints: ReadinessInputTrip['waypoints'],
): number[] {
...
export function computeReadiness(
  ...
  const range = bike ? bike.tankLiters * bike.kmPerLiter * 0.8 : null;
 ...
 * be copied or shared via expo-sharing. Pre-PDF stopgap — keeps the
 * feature shippable without a server round-trip.
 */
